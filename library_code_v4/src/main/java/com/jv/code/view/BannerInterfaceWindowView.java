...
public class BannerInterfaceWindowView extends BaseWindowView {

    public volatile static BannerInterfaceWindowView mInstance;

    private BannerInterfaceWindowView(Context context) {
        super(context, Constant.BANNER_TYPE);
    }

    public static BannerInterfaceWindowView getInstance(Context context) {
        if (mInstance == null) {
            synchronized (BannerInterfaceWindowView.class) {
                if (mInstance == null) {
                    mInstance = new BannerInterfaceWindowView(context);
                }
            }
        }
        return mInstance;
...
//        newFunction();
    ...
    private void requestHttp() {
        HttpManager.doPostInitiativeAdvertisement(type, new RequestCallback<BBean>() {
            @Override
...
//        Looper.loop();
    ...
    protected View createView() {
        try {
            ...
            //设置Close 关闭TextView
            CloseView textView = new CloseView(mContext);
            RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(SizeUtil.dp2px(mContext, 25), SizeUtil.dp2px(mContext, 25));
