...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        //获取当前下载任务中最新的 下载Id
        long completeDownloadId = intent.getLongExtra(Constant.EXTRA_DOWNLOAD_ID, -1);
        LogUtil.i("download id IDonReceive File Id: " + completeDownloadId);

        //获取下载管理对象
        DownloadManager manager = (DownloadManager) context.getSystemService(Constant.DOWNLOAD_SERVICE);

        //判断该当前下载任务完成
        if (Constant.MANAGER_DOWNLOAD_COMPLETE.equals(intent.getAction())) {

            //在广播中取出下载任务的id
            long id = intent.getLongExtra(Constant.EXTRA_DOWNLOAD_ID, 0);
            ...
            if (c.moveToFirst()) {
                //获取文件下载路径
                String filename = c.getString(c.getColumnIndex(Constant.COLUMN_LOCAL_FILENAME));
                ...
                intent_ins.setDataAndType(Uri.parse(Constant.APK_URI_FILE + filename), Constant.APK_URI_ADDRESS);
