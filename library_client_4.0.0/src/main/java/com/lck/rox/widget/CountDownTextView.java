...
public class CountDownTextView extends TextView {
    ...
    public void init() {
        //有这个属性索引 获取用户设置的颜色
        circSolidColor = BACKGROUND_COLOR;
        circFrameColor = BACKGROUND_COLOR;
        textColor = TEXT_COLOR;
        progressColor = BORDER_COLOR;
        mPaint = new Paint();
        mBounds = new Rect();
        mArcRectF = new RectF();
        mBrounds = new RectF();
...
//        canvas.drawCircle(mBounds.centerX(), mBounds.centerY(), circRadius, mPaint);
        canvas.drawRoundRect(mBrounds, circRadius, circRadius, mPaint);
...
//        canvas.drawText(mText, mCenterX, textY, text_paint);
        if (progress != 0) {
            int s = (int) (100 / (timeMillis / 1000));
            int time = progress / s;

            canvas.drawText("跳过\t|\t" + time, mCenterX, textY, text_paint);
        } else {
            canvas.drawText("跳过\t|\t" + 0, mCenterX, textY, text_paint);
        }

        Log.i("time", "time:" + progress);
